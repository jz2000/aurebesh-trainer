import * as sessionMem from './session-mem'

var lettersPool = [];

export function resetStatistics(tasks) {
    lettersPool = [];
    var letters = tasks;
...
export function findWorstLetter(activeMems) {
...
export function collectWorstLetters(letters, worstRatio) {
    var result = [];
    for (var i = 0; i < letters.length; i++) {
        var letter = letters[i];
        var ratio = letter.hitsAfterLastMiss;
...
export function collectWellLearntLetters() {
    var result = [];
...
export function collectLettersToLearn() {
    var result = [];
    for (var i = 0; i < lettersPool.length; i++) {
        var letter = lettersPool[i];
        if (letter.hitsAfterLastMiss < 20) {
